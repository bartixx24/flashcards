...
import com.example.flashcards.workers.UpdateSetWithFlashcardsChangedWorker
import kotlinx.coroutines.launch

private const val TAG = "FlashcardsViewModel"

class FlashcardsViewModel(private val flashcardsDao: FlashcardsDao, application: Application): ViewModel() {

    companion object {
        // create flashcard input data keys
        const val CREATE_FLASHCARD = "create_flashcard"
    ...
    fun createFlashcard(term: String, definition: String, learned: Boolean): Boolean {
        ...
        val updateSetWithFlashcardsChangedRequest = OneTimeWorkRequestBuilder<UpdateSetWithFlashcardsChangedWorker>().build()

        var workRequestChain = workManager
            .beginUniqueWork(
                CREATE_FLASHCARD,
                ExistingWorkPolicy.APPEND,
                addFlashcardWorkRequest
            )

        workRequestChain = workRequestChain.then(updateSetWithFlashcardsChangedRequest)

        workRequestChain.enqueue()
